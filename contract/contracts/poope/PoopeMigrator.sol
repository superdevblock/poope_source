...
import "./interfaces/IPoopeMigrator.sol";

contract PoopeMigrator is ReentrancyGuard, IPoopeMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "PoopeMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "PoopeMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "PoopeMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "PoopeMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "PoopeMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "PoopeMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "PoopeMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "PoopeMigrator: invalid _lpTokenBs.length");

        isInitialized = true;

        ammRouter = _ammRouter;
        poopePrice = _poopePrice;
    ...
    function migrate(
        ...
        require(isMigrationActive, "PoopeMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "PoopeMigrator: token not whitelisted");
        require(_tokenAmount > 0, "PoopeMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "PoopeMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(poopePrice);
        require(mintAmount > 0, "PoopeMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "PoopeMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "PoopeMigrator: invalid tokenA");
            require(tokenB != address(0), "PoopeMigrator: invalid tokenB");
        ...
        IPoopeIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "PoopeMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "PoopeMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "PoopeMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "PoopeMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("PoopeMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "PoopeMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "PoopeMigrator: invalid _action");
