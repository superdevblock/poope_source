...
contract EsBluBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esBlu;

    constructor(address _esBlu) {
        admin = msg.sender;
        esBlu = _esBlu;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsBluBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esBlu).transferFrom(msg.sender, _accounts[i], _amounts[i]);
