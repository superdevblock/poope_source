...
contract EsPoopeBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esPoope;

    constructor(address _esPoope) {
        admin = msg.sender;
        esPoope = _esPoope;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsPoopeBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esPoope).transferFrom(msg.sender, _accounts[i], _amounts[i]);
