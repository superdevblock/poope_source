...
import "./interfaces/IBluMigrator.sol";

contract BluMigrator is ReentrancyGuard, IBluMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "BluMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "BluMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "BluMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "BluMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "BluMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "BluMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "BluMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "BluMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "BluMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "BluMigrator: token not whitelisted");
        require(_tokenAmount > 0, "BluMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "BluMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(bluPrice);
        require(mintAmount > 0, "BluMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "BluMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "BluMigrator: invalid tokenA");
            require(tokenB != address(0), "BluMigrator: invalid tokenB");
        ...
        IBluIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "BluMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "BluMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "BluMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "BluMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("BluMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "BluMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "BluMigrator: invalid _action");
