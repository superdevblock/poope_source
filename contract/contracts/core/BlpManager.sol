...
contract BlpManager is ReentrancyGuard, Governable, IBlpManager {
    ...
    constructor(address _vault, address _usdg, address _blp, uint256 _cooldownDuration) {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        blp = _blp;
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minBlp) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("BlpManager: action not enabled"); }
        return _addLiquidity(msg.sender, msg.sender, _token, _amount, _minUsdg, _minBlp);
    }

    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minBlp) external override nonReentrant returns (uint256) {
        _validateHandler();
        return _addLiquidity(_fundingAccount, _account, _token, _amount, _minUsdg, _minBlp);
    }
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minBlp) private returns (uint256) {
        ...
        require(usdgAmount >= _minUsdg, "BlpManager: insufficient USDG output");

        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(blpSupply).div(aumInUsdg);
        ...
        emit AddLiquidity(_account, _token, _amount, aumInUsdg, blpSupply, usdgAmount, mintAmount);
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _blpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        uint256 usdgAmount = _blpAmount.mul(aumInUsdg).div(blpSupply);
        ...
        emit RemoveLiquidity(_account, _tokenOut, _blpAmount, aumInUsdg, blpSupply, usdgAmount, amountOut);
