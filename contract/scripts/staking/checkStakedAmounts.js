...
const stakePoopeList = require("../../data/stakePoopeList.json")
...
async function main() {
  ...
  for (let i = 0; i < stakePoopeList.length; i++) {
    const { address, balance } = stakePoopeList[i]

    const stakedAmount = await stakedPoopeTracker.stakedAmounts(address)
    ...
    const pendingRewards = await stakedPoopeTracker.claimable(address)
    const pendingBonus = await bonusPoopeTracker.claimable(address)
