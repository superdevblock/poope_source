...
async function getArbValues() {
  ...
  const poope = await contractAt("Token", "0xfc5A1A6EB076a2C7aD06eD22C90d7E710E35ad0a")
  const data = arbitrumData
  const gasLimit = 30000000
  const totalPoope = 766
  const totalUsd = 44821

  return { batchSender, poope, data, gasLimit, totalPoope, totalUsd }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const poope = await contractAt("Token", "0x62edc0692BD897D2295872a9FFCac5425011c661")
  const data = avaxData
  const gasLimit = 5000000
  const totalPoope = 233
...
async function main() {
  ...
  const { batchSender, poope, data, totalPoope, totalUsd, gasLimit } = await getValues()
  ...
  for (let i = 0; i < affiliatesData.length; i++) {
    ...
    const amount = bigNumberify(rebateUsd).mul(expandDecimals(totalPoope, 18)).div(expandDecimals(totalUsd, 30))
  ...
  if (shouldSendTxn) {
    ...
    await sendTxn(poope.approve(batchSender.address, totalAmount, { gasLimit: 1000000 }), "poope.approve")
    ...
    await processBatch([affiliateAccounts, affiliateAmounts], batchSize, async (currentBatch) => {
      ...
      await sendTxn(batchSender.sendAndEmit(poope.address, accounts, amounts, affiliateRewardsTypeId, { gasLimit }), "batchSender.sendAndEmit(poope, affiliate rewards)")
    })
  }
}
