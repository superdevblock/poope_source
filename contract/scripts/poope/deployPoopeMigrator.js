...
async function main() {
  ...
  const poopeMigrator = await deployContract("PoopeMigrator", [2])
  const gmtPoopeIou = await deployContract("PoopeIou", [poopeMigrator.address, "GMT POOPE (IOU)", "GMT:POOPE"])
  const xgmtPoopeIou = await deployContract("PoopeIou", [poopeMigrator.address, "xGMT POOPE (IOU)", "xGMT:POOPE"])
  const gmtUsdgPoopeIou = await deployContract("PoopeIou", [poopeMigrator.address, "GMT-USDG POOPE (IOU)", "GMT-USDG:POOPE"])
  const xgmtUsdgPoopeIou = await deployContract("PoopeIou", [poopeMigrator.address, "xGMT-USDG POOPE (IOU)", "xGMT-USDG:POOPE"])
  ...
  const poopePrice = bigNumberify(2 * precision)
  ...
  const iouTokens = [gmtPoopeIou.address, xgmtPoopeIou.address, gmtUsdgPoopeIou.address, xgmtUsdgPoopeIou.address]
  ...
  await poopeMigrator.initialize(
    ammRouter.address,
    poopePrice,
    signers,
    whitelistedTokens,
