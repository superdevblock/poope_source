...
async function main() {
  ...
  const bluMigrator = await deployContract("BluMigrator", [2])
  const gmtBluIou = await deployContract("BluIou", [bluMigrator.address, "GMT BLU (IOU)", "GMT:BLU"])
  const xgmtBluIou = await deployContract("BluIou", [bluMigrator.address, "xGMT BLU (IOU)", "xGMT:BLU"])
  const gmtUsdgBluIou = await deployContract("BluIou", [bluMigrator.address, "GMT-USDG BLU (IOU)", "GMT-USDG:BLU"])
  const xgmtUsdgBluIou = await deployContract("BluIou", [bluMigrator.address, "xGMT-USDG BLU (IOU)", "xGMT-USDG:BLU"])
  ...
  const bluPrice = bigNumberify(2 * precision)
  ...
  const iouTokens = [gmtBluIou.address, xgmtBluIou.address, gmtUsdgBluIou.address, xgmtUsdgBluIou.address]
