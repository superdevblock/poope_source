...
async function main() {
  ...
  const bluMigrator = await contractAt("BluMigrator", "0x0472F402EA8E301D7595545884Ad4C420E9865d6")
  const gmt = {
    name: "GMT",
  ...
  if (shouldApprove) {
    for (let i = 0; i < tokens.length; i++) {
      ...
      const migratedAmount = await bluMigrator.migratedAmounts(account, token.contract.address)
      ...
      await sendTxn(bluMigrator.setMaxMigrationAmount(account, token.contract.address, totalAmount), message)
    }
  }
}
