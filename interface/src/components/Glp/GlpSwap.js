...
import { ARBITRUM, CRONOS, getChainName, IS_NETWORK_DISABLED } from "config/chains";
...
export default function GlpSwap(props) {
  ...
  const buyGlp = () => {
    ...
    const method = swapTokenAddress === AddressZero ? "mintAndStakePLPETH" : "mintAndStakePLP";
  ...
  const sellGlp = () => {
    ...
    const contract = new ethers.Contract(rewardRouterAddress, RewardRouter.abi, library.getSigner());
    const method = swapTokenAddress === AddressZero ? "unstakeAndRedeemPLPETH" : "unstakeAndRedeemPLP";
    const params =
  ...
  return (
    <div className="GlpSwap">
      ...
      {/* <div className="Page-title-section">
        <div className="Page-title">{isBuying ? "Buy PLP" : "Sell PLP"}</div>
        ...
        {!isBuying && <div className="Page-description">
          Redeem your PLP tokens for any supported asset.
          {inCooldownWindow && <div>
            PLP tokens can only be redeemed 15 minutes after your most recent purchase.<br/>
            Your last purchase was at {formatDateTime(lastPurchaseTime)}, you can redeem PLP tokens after {formatDateTime(redemptionTime)}.<br/>
          </div>}
          <div>View <Link to="/earn">staking</Link> page.</div>
        </div>}
      </div> */}
          ...
          {!isBuying && (
            ...
            >
              <div className="selected-token">
                PLP <img src={glp24Icon} alt="glp24Icon" />
              </div>
            </BuyInputSection>
          )}
