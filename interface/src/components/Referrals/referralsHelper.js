...
export async function getReferralCodeTakenStatus(account, referralCode, chainId) {
  ...
  const [ownerArbitrum, ownerAvax, ownerGoerli, ownerCronos] = await Promise.all([
    getReferralCodeOwner(ARBITRUM, referralCodeBytes32),
    getReferralCodeOwner(AVALANCHE, referralCodeBytes32),
    getReferralCodeOwner(GOERLI_TESTNET, referralCodeBytes32),
  ...
  const takenOnGoerli = 
    !isAddressZero(ownerGoerli) && (ownerGoerli !== account || (ownerGoerli === account && chainId === GOERLI_TESTNET));
  const takenOnCronos = 
  ...
  const referralCodeTakenInfo = {
    ...
    [GOERLI_TESTNET]: takenOnGoerli,
    [CRONOS]: takenOnCronos,
    both: /*takenOnGoerli && */takenOnCronos,
    ownerArbitrum,
    ownerAvax,
    ownerGoerli,
    ownerCronos,
  ...
  if (/*chainId === CRONOS ? */referralCodeTakenInfo[CRONOS]/* : referralCodeTakenInfo[GOERLI_TESTNET] */) {
...
export function getTwitterShareUrl(referralCode) {
  const message = ["Trying out trading on @bluespade, up to 100x leverage on $BTC, $ETH 📈", "For fee discounts use:"];
