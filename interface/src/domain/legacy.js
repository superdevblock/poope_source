...
import { ARBITRUM, ARBITRUM_TESTNET, AVALANCHE, GOERLI_TESTNET, CRONOS, getConstant, getHighExecutionFee } from "config/chains";
import { DECREASE, getOrderKey, INCREASE, SWAP, USD_DECIMALS } from "lib/legacy";

import { groupBy } from "lodash";
import { UI_VERSION } from "config/ui";
import { getServerBaseUrl, getServerUrl } from "config/backend";
import { getGmxGraphClient, nissohGraphClient, avalancheGraphClientForTrades, goerliGraphClientForTrades, cronosGraphClientForTrades } from "lib/subgraph/clients";
...
export function useStakedGmxSupply(library, active) {
  ...
  const gmxAddressGoerli = getContract(GOERLI_TESTNET, "GMX");
  const stakedGmxTrackerAddressGoerli = getContract(GOERLI_TESTNET, "StakedGmxTracker");

  const { data: goerliData, mutate: goerliMutate } = useSWR(
    [`StakeV2:stakedGmxSupply:${active}`, GOERLI_TESTNET, gmxAddressGoerli, "balanceOf", stakedGmxTrackerAddressGoerli],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
  ...
  let data;
  if (cronosData/* && goerliData */) {
    data = cronosData;// .add(goerliData);
  }
...
export function useGmxPrice(chainId, libraries, active) {
  ...
  const { data: gmxPriceFromGoerli, mutate: mutateFromGoerli } = useGmxPriceFromGoerli();
  const { data: gmxPriceFromCronos, mutate: mutateFromCronos } = useGmxPriceFromCronos();

  const gmxPrice = chainId === CRONOS ? gmxPriceFromCronos : gmxPriceFromGoerli;
  const mutate = useCallback(() => {
    // mutateFromAvalanche();
    // mutateFromArbitrum();
    mutateFromGoerli();
    mutateFromCronos();
  }, [/* mutateFromAvalanche, mutateFromArbitrum, */mutateFromGoerli, mutateFromCronos]);
...
export function useTotalGmxSupply() {
  ...
  // };
  const { data: gmxSupply } = useSWR([`StakeV2:totalSupply:${CRONOS}`, CRONOS, getContract(CRONOS, "GMX"), "totalSupply"], {
    fetcher: contractFetcher(undefined, Token),
  });
...
export function useTotalGmxStaked() {
  ...
  const { data: stakedGmxSupplyGoerli, mutate: updateStakedGmxSupplyGoerli } = useSWR(
    [
      `StakeV2:stakedGmxSupply:${GOERLI_TESTNET}`,
      GOERLI_TESTNET,
      getContract(GOERLI_TESTNET, "GMX"),
      "balanceOf",
      stakedGmxTrackerAddressGoerli,
    ],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  ...
  const mutate = useCallback(() => {
    ...
    updateStakedGmxSupplyGoerli();
  }, [updateStakedGmxSupplyCronos, updateStakedGmxSupplyGoerli]);

  if (stakedGmxSupplyCronos && stakedGmxSupplyGoerli) {
    let total = bigNumberify(stakedGmxSupplyCronos)/* .add(stakedGmxSupplyGoerli) */;
...
export function useTotalGmxInLiquidity() {
  ...
  let poolAddressGoerli = getContract(GOERLI_TESTNET, "TraderJoeGmxAvaxPool");
  ...
  const { data: gmxInLiquidityOnGoerli, mutate: mutateGMXInLiquidityOnGoerli } = useSWR(
    [`StakeV2:gmxInLiquidity:${GOERLI_TESTNET}`, GOERLI_TESTNET, getContract(GOERLI_TESTNET, "GMX"), "balanceOf", poolAddressGoerli],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
  ...
  const mutate = useCallback(() => {
    ...
    mutateGMXInLiquidityOnGoerli();
    mutateGMXInLiquidityOnCronos();
  }, [mutateGMXInLiquidityOnArbitrum, mutateGMXInLiquidityOnAvax, mutateGMXInLiquidityOnGoerli, mutateGMXInLiquidityOnCronos]);

  if (gmxInLiquidityOnCronos && gmxInLiquidityOnGoerli) {
    let total = bigNumberify(gmxInLiquidityOnCronos).add(gmxInLiquidityOnGoerli);
    totalGMX.current = total;
  }
  return {
    avax: gmxInLiquidityOnAvax,
    arbitrum: gmxInLiquidityOnArbitrum,
    goerli: gmxInLiquidityOnGoerli,
...
function useGmxPriceFromGoerli() {
  const poolAddress = getContract(GOERLI_TESTNET, "TraderJoeGmxAvaxPool");

  const { data, mutate: updateReserves } = useSWR(["TraderJoeGmxAvaxReserves", GOERLI_TESTNET, poolAddress, "getReserves"], {
    fetcher: contractFetcher(undefined, UniswapV2),
  });
  const { _reserve0: gmxReserve, _reserve1: avaxReserve } = data || {};

  const vaultAddress = getContract(GOERLI_TESTNET, "Vault");
  const wethAddress = getTokenBySymbol(GOERLI_TESTNET, "WETH").address;
  const { data: ethPrice, mutate: updateEthPrice } = useSWR(
    [`StakeV2:ethPrice`, GOERLI_TESTNET, vaultAddress, "getMinPrice", wethAddress],
    {
      fetcher: contractFetcher(undefined, Vault),
    }
  );
  ...
  const PRECISION = bigNumberify(10).pow(18);
  let gmxPrice;
  if (avaxReserve && gmxReserve && ethPrice) {
    gmxPrice = avaxReserve.mul(PRECISION).div(gmxReserve).mul(ethPrice).div(PRECISION);
  }
...
function useGmxPriceFromCronos() {
  ...
  const { _reserve0: croReserve, _reserve1: gmxReserve } = data || {};
  ...
  if (croReserve && gmxReserve && croPrice) {
    ...
    // for presale
    gmxPrice = bigNumberify(15).mul(bigNumberify(10).pow(28));
  }
