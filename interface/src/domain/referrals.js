...
import { arbitrumReferralsGraphClient, avalancheReferralsGraphClient, goerliReferralsGraphClient, cronosReferralsGraphClient } from "lib/subgraph/clients";
...
const ACTIVE_CHAINS = [/* ARBITRUM, AVALANCHE, GOERLI_TESTNET, */CRONOS];
...
export function useUserCodesOnAllChain(account) {
  ...
  useEffect(() => {
    async function main() {
      const [arbitrumCodes, avalancheCodes, goerliCodes, cronosCodes] = await Promise.all(
      ...
      const [/*codeOwnersOnAvax = [], codeOwnersOnArbitrum = [], codeOwnersOnGoerli = [],*/ codeOwnersOnCronos = []] = await Promise.all([
        // getCodeOwnersData(AVALANCHE, account, arbitrumCodes),
        // getCodeOwnersData(ARBITRUM, account, avalancheCodes),
        // getCodeOwnersData(GOERLI_TESTNET, account, goerliCodes),
      ...
      setData({
        ...
        //   return acc;
        // }, {}),
        // [GOERLI_TESTNET]: codeOwnersOnGoerli.reduce((acc, cv) => {
        //   acc[cv.code] = cv;
        //   return acc;
        // }, {}),
