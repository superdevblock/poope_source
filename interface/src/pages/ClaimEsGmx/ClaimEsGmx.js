...
const VEST_WITH_GMX_GOERLI = "VEST_WITH_GMX_GOERLI";
const VEST_WITH_GLP_GOERLI = "VEST_WITH_GLP_GOERLI";
...
export default function ClaimEsGmx({ setPendingTxns }) {
  ...
  const goerliRewardReaderAddress = getContract(GOERLI_TESTNET, "RewardReader");
  ...
  const goerliVesterAdddresses = [getContract(GOERLI_TESTNET, "GmxVester"), getContract(GOERLI_TESTNET, "GlpVester")];
  ...
  const { data: goerliVestingInfo } = useSWR(
    [
      `StakeV2:vestingInfo:${active}`,
      GOERLI_TESTNET,
      goerliRewardReaderAddress,
      "getVestingInfoV2",
      account || PLACEHOLDER_ACCOUNT,
    ],
    {
      fetcher: contractFetcher(undefined, RewardReader, [goerliVesterAdddresses]),
    }
  );
  ...
  const goerliVestingData = getVestingDataV2(goerliVestingInfo);
  ...
  if (selectedOption === VEST_WITH_GLP_GOERLI && goerliVestingData) {
    const result = getVestingValues({
      minRatio: bigNumberify(320),
      amount,
      vestingDataItem: goerliVestingData.glpVester,
    });
  ...
  const claim = () => {
    ...
    if (selectedOption === VEST_WITH_GMX_GOERLI) {
      receiver = "0x5d2E4189d0b273d7E7C289311978a0183B96C404";
    }

    if (selectedOption === VEST_WITH_GLP_GOERLI) {
      receiver = "0x5d2E4189d0b273d7E7C289311978a0183B96C404";
    }
          ...
          <div>
            ...
            <div className="ClaimEsGmx-vesting-options">
              ...
              {/* <Checkbox
                ...
                isChecked={selectedOption === VEST_WITH_GMX_GOERLI}
              ...
              <Checkbox
                className="goerli btn btn-primary btn-left btn-lg"
                isChecked={selectedOption === VEST_WITH_GLP_GOERLI}
