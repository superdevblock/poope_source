...
export function handleAnswerUpdatedCRO(event: AnswerUpdatedEvent): void {
  _storeChainlinkPrice(WCRO, event.params.current, event.block.timestamp)
}
...
export function handleAnswerUpdatedATOM(event: AnswerUpdatedEvent): void {
  _storeChainlinkPrice(ATOM, event.params.current, event.block.timestamp)
}

export function handleAnswerUpdatedADA(event: AnswerUpdatedEvent): void {
  _storeChainlinkPrice(ADA, event.params.current, event.block.timestamp)
}

export function handleAnswerUpdatedDOGE(event: AnswerUpdatedEvent): void {
  _storeChainlinkPrice(DOGE, event.params.current, event.block.timestamp)
}
...
// export function handleUniswapBluEthSwap(event: UniswapSwap): void {
//   let ethPerBlu = -(event.params.amount0 * BigInt.fromI32(10).pow(18) / event.params.amount1) * BigInt.fromI32(100) / BigInt.fromI32(99)
//   let bluPrice = getTokenAmountUsd(WETH, ethPerBlu)

//   let totalId = BLU
//   _storeUniswapPrice(totalId, BLU, bluPrice, "last", event.block.timestamp)

//   let id = BLU + ":" + event.block.timestamp.toString()
//   _storeUniswapPrice(id, BLU, bluPrice, "any", event.block.timestamp)
// }
