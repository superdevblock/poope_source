...
import {
  ...
  AddLiquidity,
  RemoveLiquidity
} from "../generated/BlpManager/BlpManager"

// import {
//   Distribute
// } from "../generated/FeeBluRewardDistributor/RewardDistributor"
...
const LIQUIDATOR_ADDRESS = "0x5d2e4189d0b273d7e7c289311978a0183b96c404"
...
function _storeLiquidatedPosition(
  ...
  let averagePrice = (position !== null) ? position.averagePrice : BigInt.zero()
  ...
  liquidatedPosition.collateral = (position !== null) ? position.collateral : BigInt.zero()
  ...
  let accruedFundingRate = BigInt.fromI32(fundingRateEntity !== null ? fundingRateEntity.endFundingRate : 0) - (position ? position.entryFundingRate : BigInt.zero())
...
export function handleAddLiquidity(event: AddLiquidity): void {
  _storeBlpStat(event.block.timestamp, event.params.blpSupply, event.params.aumInUsdg)
}
...
// export function handleDistributeEthToBlu(event: Distribute): void {
//   let amount = event.params.amount
//   let amountUsd = getTokenAmountUsd(WETH, amount)
//   let totalEntity = _getOrCreateBluStat("total", "total")
...
//   let entity = _getOrCreateBluStat(id, "daily")
...
//   entity.save()
// }
// export function handleDistributeEsbluToBlu(event: Distribute): void {
//   let amount = event.params.amount
//   let amountUsd = getTokenAmountUsd(BLU, amount)

//   let totalEntity = _getOrCreateBluStat("total", "total")
//   totalEntity.distributedEsblu += amount
//   totalEntity.distributedEsbluCumulative += amount
//   totalEntity.distributedEsbluUsd += amountUsd
//   totalEntity.distributedEsbluUsdCumulative += amountUsd
...
//   entity.distributedEsblu += amount
//   entity.distributedEsbluCumulative = totalEntity.distributedEthCumulative
//   entity.distributedEsbluUsd += amountUsd
//   entity.distributedEsbluUsdCumulative = totalEntity.distributedUsdCumulative
...
function _getOrCreateBluStat(id: string, period: string): BluStat {
  let entity = BluStat.load(id)
  if (entity === null) {
    entity = new BluStat(id)
    ...
    entity.distributedEsblu = ZERO
    entity.distributedEsbluCumulative = ZERO
    entity.distributedEsbluUsd = ZERO
    entity.distributedEsbluUsdCumulative = ZERO
    entity.period = period
  }
  return entity as BluStat
}
...
function _storeVolumeBySource(type: string, timestamp: BigInt, source: Address | null, volume: BigInt): void {
  let id: string;
  if (source !== null)
    id = _getHourId(timestamp) + ":" + source.toHexString()
  else
    id = _getHourId(timestamp) + ":"
...
function _getOrCreateBlpStat(id: string, period: string, periodTimestmap: i32): BlpStat {
  let entity = BlpStat.load(id)
  if (entity === null) {
    entity = new BlpStat(id)
    entity.period = period
    entity.blpSupply = ZERO
  ...
  return entity as BlpStat
}

// export function handleDistributeEthToBlp(event: Distribute): void {
//   let amount = event.params.amount
//   let amountUsd = getTokenAmountUsd(WETH, amount)
//   let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32

//   let totalEntity = _getOrCreateBlpStat("total", "total", periodTimestamp)
//   totalEntity.distributedEth += amount
//   totalEntity.distributedEthCumulative += amount
//   totalEntity.distributedUsd += amountUsd
//   totalEntity.distributedUsdCumulative += amountUsd
...
//   let id = periodTimestamp.toString() + ":daily"
//   let entity = _getOrCreateBlpStat(id, "daily", periodTimestamp)
...
// export function handleDistributeEsbluToBlp(event: Distribute): void {
//   let amount = event.params.amount
//   let amountUsd = getTokenAmountUsd(BLU, amount)
//   let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32
...
function _storeBlpStat(timestamp: BigInt, blpSupply: BigInt, aumInUsdg: BigInt): void {
  let periodTimestamp = parseInt(_getDayId(timestamp)) as i32
  let totalEntity = _getOrCreateBlpStat("total", "total", periodTimestamp)
  totalEntity.aumInUsdg = aumInUsdg
  totalEntity.blpSupply = blpSupply
  totalEntity.save()

  let id = periodTimestamp.toString() + ":daily"
  let entity = _getOrCreateBlpStat(id, "daily", periodTimestamp)
  entity.aumInUsdg = aumInUsdg
  entity.blpSupply = blpSupply
  entity.save()
}
